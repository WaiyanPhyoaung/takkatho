...
<Aside title="ရည်ရွယ်ချက်" type="tip">
  ...
  ကို ပေါင်းစပ်ပြီး ဘယ်လို အလုပ်လုပ်သလဲဆိုတာ ပြသသွားမယ်။
</Aside>
...
- ပြီးတော့ အပြင်ကနေ ရလာမယ့် Setting တွေကို 'အတု' ဖန်တီးပြီး ထည့်သွင်းမယ်။ (တကယ့် Project မှာဆိုရင် ဒါတွေက JSON File ထဲကနေ ဒါမှမဟုတ် Database ထဲကနေ လာနိုင်ပါတယ်)။
...
ပြီးတော့ Setting တွေ မပါလာရင် အစားထိုးပေးမယ့် 'Default Values' တွေ (ပုံမှန်တန်ဖိုးတွေ) ကို သတ်မှတ်ပါမယ်။ ဒီ `defaultConfig` ကို `Partial<AppConfig>` လို့ သတ်မှတ်ထားတာကို သတိထားပါ။
...
- **Generics (`<T, D>`):**
  ...
  - `D` ဆိုတာကတော့ Default Values တွေရဲ့ ပုံစံ (Type) ပါ။ `D` က `T` ရဲ့ တစ်စိတ်တစ်ပိုင်း (`Partial`) ဖြစ်ရမယ်လို့ သတ်မှတ်ထားတယ်။
- **Input Parameters:**
...
- ဒါ့အပြင် `AppConfig` ထဲမှာ မပါဝင်ပေမဲ့ `customServiceUrl` လိုမျိုး အပို Properties တွေကိုလည်း ဒီ `configFileContent` ထဲမှာ ထည့်ခွင့်ပြုထားတာဖြစ်ပါတယ်။
