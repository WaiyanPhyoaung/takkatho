...
## Utility Types များ အမြန်ကြည့်နိုင်ရန် (Cheat Sheet)

Utility Types တွေရဲ့ အဓိက အချက်အလက်တွေကို အတိုချုပ် စာရင်း ပြန်လုပ်ပေးထားပါတယ်။

- **`Partial<T>`: All properties optional.**<br/>
  `T` ရဲ့ Properties အားလုံးကို Optional (ပါချင်မှပါ) ဖြစ်အောင် လုပ်ပေးပါတယ်။
  <br />
  <br />
- **`Required<T>`: All properties required.**<br/> 
  `T` ရဲ့ Properties အားလုံးကို မဖြစ်မနေ (Required) ဖြစ်အောင် လုပ်ပေးပါတယ်။
  <br />
  <br />
- **`Readonly<T>`: All properties readonly.**<br/> 
  `T` ရဲ့ Properties အားလုံးကို ပြင်လို့မရအောင် (Readonly) လုပ်ပေးပါတယ်။
  <br />
  <br />
- **`Pick<T, K>`: Selects properties K from T.**<br/> 
  `T` ထဲက ကိုယ်လိုချင်တဲ့ Property `K` တွေကို ရွေးထုတ်ပေးပါတယ်။
  <br />
  <br />
- **`Omit<T, K>`: Removes properties K from T.**<br/> 
  `T` ထဲက ကိုယ်မလိုချင်တဲ့ Property `K` တွေကို ဖယ်ထုတ်ပေးပါတယ်။
  <br />
  <br />
- **`Record<K, T>`: Object with keys of type K and values of type T.**<br/> 
  Keys တွေက `K` Type ဖြစ်ပြီး Values တွေက `T` Type ဖြစ်တဲ့ Object တစ်ခုကို လုပ်ပေးပါတယ်။
  <br />
  <br />
- **`Exclude<T, U>`: From union T, remove types assignable to U.**<br/> 
  Union Type `T` ထဲက `U` နဲ့ တူတဲ့ Types တွေကို ဖယ်ထုတ်ပေးပါတယ်။
  <br />
  <br />
- **`Extract<T, U>`: From union T, keep only types assignable to U.**<br/> 
  Union Type `T` ထဲက `U` နဲ့ တူတဲ့ Types တွေကိုပဲ ထားပေးပါတယ်။
  <br />
  <br />
- **`NonNullable<T>`: Remove null and undefined from T.**<br/> 
  `T` ထဲက `null` နဲ့ `undefined` Types တွေကို ဖယ်ထုတ်ပေးပါတယ်။
  <br />
  <br />
- **`Parameters<F>`: Tuple of function F's parameter types.**<br/> 
  Function `F` ရဲ့ Parameters တွေရဲ့ Type ကို Tuple အနေနဲ့ ထုတ်ပေးပါတယ်။
  <br />
  <br />
- **`ReturnType<F>`: Return type of function F.**<br/> 
  Function `F` ကနေ ပြန်လာမယ့် (Return) Type ကို ထုတ်ပေးပါတယ်။
  <br />
  <br />
- **`Awaited<P>`: Resolved type of Promise P.**<br/> 
  Promise `P` ရဲ့ အထဲက ပြန်လာမယ့် (Resolved) Type ကို ထုတ်ပေးပါတယ်။
